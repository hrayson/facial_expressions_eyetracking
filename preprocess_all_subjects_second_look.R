...
for(i in 1:length(subj_ids)) {
	...
	for(j in 1:length(trial_ids)) {
		...
		if(length(first_look_emotion)>0 && trial_data$Emotion[first_look_emotion] && length(first_look_neutral)>0 && trial_data$Neutral[first_look_neutral]) {
			# If looked at emotion face first - set emotion to false
			...
			# If looked at neutral face first - set neutral to false
			else {
...
second_look_data$Neutral[second_look_data$Trackloss == TRUE] <- NA
second_look_data$Emotion[second_look_data$Trackloss == TRUE] <- NA
...
write.csv(second_look_data, file = "/data/child_anxiety/EyeTracking/data/all_subjects_second_look.csv")
